...
bool RFM69::switch_frequencyMHz(float freqMHz)
{
    //noInterrupts();
    //setMode(RF69_MODE_STANDBY);
    ...
    //receiveBegin();
    while( !(readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_PLLLOCK));
...
bool RFM69::noRx()
{
    return (_mode == RF69_MODE_RX && DATALEN == 0 && readRSSI() < CSMA_LIMIT);
}
...
void RFM69::sendFrame(const void* buffer, byte bufferSize)
{
    ...
    #if (F_CPU >1000000L)
    delayMicroseconds(1000); // need a delay because the FIFONOTEMPTY goes low before the packet is sent. this workarounf works up to 8 MHz clock
    #endif
...
{
    #if defined(__AVR_ATmega3208__) || defined(__AVR_ATmega4808__)
    if (!digitalRead(_interruptPin)) // detect rising edge
        return; // if falling edge, return immediately
    #endif
...
void RFM69::select()
{
    STATUSREG= SREG;
    noInterrupts();
    #if defined(__AVR_ATtiny84__) || defined(__AVR_ATtiny44__) || defined(MEGACOREX) || defined(ARDUINO_avrdd)|| defined(ARDUINO_avrda)// tiny doesn't set SPI settings like this
...
{
    digitalWrite(_slaveSelectPin, HIGH);
    #if defined(__AVR_ATtiny84__) || defined(__AVR_ATtiny44__) || defined(MEGACOREX) || defined(ARDUINO_avrdd)|| defined(ARDUINO_avrda)
    #else
