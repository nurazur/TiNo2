...
/*
  ...
  Changes introduced by nurazur:
  - cast paramters passed to requestFrom function in order to remove lots ow compiler warnings.
...
float HTU21D::readHumidity(HTU21D_HUMD_OPERATION_MODE sensorOperationMode)
{
  ...
  do
  {
    ...
    Wire.requestFrom((uint8_t)HTU21D_ADDRESS, (size_t)3, true);  //true = stop message after transmission & releas the I2C bus
  }
...
float HTU21D::readTemperature(HTU21D_TEMP_OPERATION_MODE sensorOperationMode)
{
  ...
  do
  {
    ...
    Wire.requestFrom((uint8_t)HTU21D_ADDRESS, (size_t)qntRequest, true);             //true = stop message after transmission & releas the I2C bus
  }
...
uint8_t HTU21D::readFirmwareVersion(void)
{
  ...
  Wire.requestFrom((uint8_t)HTU21D_ADDRESS, (size_t)1, true); //true = stop message after transmission & releas the I2C bus
  #if ARDUINO >= 100
