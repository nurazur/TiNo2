        ...
        if len(items) > 2:
            ...
            if thekey in list(mem.keys()): #verbose
                ...
                if (thekey in sensors):
                    ...
                    addr = mem[bitfield]
                    set_sensors = True
                elif thekey in pciconfig and ("PCI" in bitfield):
                    addr = mem[bitfield]
                    set_pciconfig = True
                else:
            ...
            if addr in list(memtypes.keys()):
                if memtypes[addr] == 'b':  #signed char
                    ...
                    if set_pciconfig:
                        ...
                        #print(thekey)
                        #print(bitfield)
                        pcinum = ord(bitfield[3])-ord('0')
                        if thekey == pciconfig[0]: #Trigger
                            if (pcinum >=0 and pcinum < 4):
                                #print (pcinum)
                                print (pciconfig_val[pcinum])
                                pciconfig_val[pcinum] &=  ~(0x07) # 3 bits
                                pciconfig_val[pcinum] |= val
                                outstr = b"w,%i,%i\n" % (addr, pciconfig_val[pcinum])
                                print(outstr.decode())
                                serial.write(outstr)
                            else:
                                print("ERROR: no valid PCI Number")
                                return
                        elif thekey == pciconfig[1]: #Mode
                            if (pcinum >=0 and pcinum < 4):
                                print (pciconfig_val[pcinum])
                                pciconfig_val[pcinum] &= 0x07 # 5 bits
        ...
        if t == 'b': # signed byte
            ...
            if "SENSORCONFIG" in mem[i]:
                ...
                if (pcinum >=0 and pcinum < 4):
                    ...
                    outp  = ("%s.%s=%i") % (mem[i], pciconfig[0], pciconfig_val[pcinum] & 0x07)
                    outp += ("\n%s.%s=%i") % (mem[i], pciconfig[1], (pciconfig_val[pcinum] & 0x18)>>3)
        ...
        if len(items) > 1:
            ...
            if items[0] in list(mem.keys()):
                ...
                if items[0] in sensors:             #
                    ...
                    addr = mem[bitfield]
                    set_sensors = True
                elif items[0] in pciconfig and ("PCI" in bitfield):  #pciconfig: TRIGGER or MODE
                    addr = mem[bitfield]
                    set_pciconfig = True
                else:
                    continue # ignore this item, it is not in sensors
            else:
                continue
            ...
            if memtypes[addr] == 'b': #signed char
                ...
                if set_sensors and 0 <= val and val <= 1:
                    ...
                    #print("item: %s, at index: %i, value old: 0x%X, new value at index: %i" % (items[0],i, sensors_val, val))
                    # set Flag at position i to 0
                    sensors_val = sensors_val & ~(1<<i)
                    sensors_val |= (val<<i)
                    outstr = b"w,%i,%i\n" % (addr, sensors_val)
                    ...
                    pcinum = ord(bitfield[3])-ord('0')
                    if items[0] == pciconfig[0] and 0 <= val and val <= 4: #Trigger
                        print ("PCI%i TRIGGER, value old: %i " % (pcinum, pciconfig_val[pcinum]))
                        pciconfig_val[pcinum] &=  ~(0x07) # 3 bits
                        pciconfig_val[pcinum] |= val
                        outstr = b"w,%i,%i\n" % (addr, pciconfig_val[pcinum])
                        print(outstr.decode())
      ...
      try:
        if kbhit():
            ...
            if ord(ch) == 10 or ord(ch) == 13:
                ...
                elif inputstr == b'monoff' or inputstr==b'moff':
        ...
        if port.isOpen() and port.inWaiting() > 0:
            ...
            for recstr in recstrlist:
                if len(recstr) >  0:
                    if recstr ==b"CAL?": # old TiNo
                        port.write(b'y')
                        #print "received calibration request and sent 'y'"
                        #print time.time() -ts
                    elif recstr==b"    ": # 4 spaces #TiNo2
                        port.write(b'y')
